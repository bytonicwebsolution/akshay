        ...
        products.map(async (product) => {
          ...
          if (product.variant) {
            ...
            totalStock = productStocks.reduce(
              (sum, stock) =>
                sum + stock.current_stock.reduce((a, b) => a + b, 0),
              0
            );
          } else {
            ...
            totalStock = totalStock
              ? totalStock.current_stock.reduce((a, b) => a + b, 0)
              : 0;
          }
          return {
            ...product._doc,
    ...
    try {
      ...
      for (const attributeSet of attributeSetIdArr) {
        ...
        if (attributeValues.length > 0) {
          allAttributeValues = allAttributeValues.concat(attributeValues);
        }
        html +=
      ...
      return res.status(500).send({
        message: "Error fetching Prodcuts Attribute Values: " + error.message,
      });
    }
  };
      ...
      if (attributeSetIds) {
        ...
        for (const [index, variant] of Object.entries(all_variants)) {
          ...
          for (const [key, item] of Object.entries(variant)) {
            const attributeValue = await AttributeValue.findById(item);
          ...
          if (Array.isArray(req.body.variant_name)) {
            const index = req.body.variant_name.indexOf(variantTitle);
            if (index !== -1) {
          ...
          if (req.body.loadtime) {
            ...
            if (existingStock) {
              if (Array.isArray(existingStock.name)) {
                const indexEdit = existingStock.name.indexOf(variantTitle);
                if (indexEdit !== -1) {
                  price = existingStock.price[indexEdit];
                  current_stock = existingStock.current_stock[indexEdit];
                  variantSKU = existingStock.sku[indexEdit];
                  variantIds = existingStock.attribute_value_id[indexEdit];
          ...
          html += '<label class="font-normal">' + variantTitle + "</label>";
          html +=
          ...
          if (image) {
            const filePath = path.join(root, "/public/dist/product/", image);
            if (fs.existsSync(filePath)) {
              html += '<div class="mb-2 mt-2" id="previewthumbnailicon" >';
              html +=
    ...
    try {
      upload(req, res, async function (err) {
        ...
        const insertRecord = new Product({
          ...
          slug: data.slug,
          status_id:
            data.status_id === "on" ? activeStatus._id : inactiveStatus._id,
        ...
        if (data.variant === "on") {
          ...
          const variant_name = Array.isArray(data.variant_name)
            ? data.variant_name
            : [data.variant_name];
          const skus = Array.isArray(data.sku) ? data.sku : [data.sku];
          ...
          const filteredvVariantName = variant_name.filter((val) => val);
          const filteredCurrentStocks = currentStocks.filter((val) => val);
    ...
    try {
      editupload(req, res, async function (err) {
        ...
        if (data.variant === "on") {
          ...
          if (existingStock) {
            return res.status(400).send({
              message: `SKU ${data.sku} must be unique. This SKU exists.`,
            });
          }
        ...
        await Product.findByIdAndUpdate({ _id: req.body.editid }, updatedData);
        return res.send({
          success: true,
          status: 200,
  ...
  static delete = async (req, res) => {
    try {
      if (Array.isArray(req.body.ids)) {
        ...
        return res.status(400).send({
          message: "Invalid request. Provide a product ID or a list of IDs.",
        });
      }
    } catch (error) {
      console.log(error);
