...
class WebsiteController {
  ...
  static products_by_category = async (req, res) => {
    try {
      ...
      const findChildCategories = async (categoryId) => {
        ...
        let categoryIds = categories.map((category) => category._id.toString());

        for (const category of categories) {
          const childCategoryIds = await findChildCategories(category._id);
        ...
        products.map(async (product) => {
          ...
          if (!product.variant) {
            ...
            if (productStock) {
              ...
              responseObject.current_stock = productStock.current_stock[0];
            }
          }
  ...
  static get_all_products = async (req, res) => {
    try {
      ...
      const productIdsFromProducts = products.map((product) => product._id);
      ...
      const productsWithSpecialPrice = products.map((product) => {
        ...
        if (productData.thumbnail && productData.thumbnail.trim() !== "") {
          fs.access(thumbnailPath, fs.constants.F_OK, (err) => {
            if (err) {
              ...
              productData.thumbnail = mediaUrl + productData.thumbnail.trim();
            }
          });
        } else {
          productData.thumbnail = defaultImage;
        ...
        productData.gallery_image = productData.gallery_image
          ?.map((image) => {
            ...
            try {
              if (image && image.trim() !== "" && fs.existsSync(imagePath)) {
    ...
    try {
      ...
      if (productData.thumbnail && productData.thumbnail.trim() !== "") {
        fs.access(thumbnailPath, fs.constants.F_OK, (err) => {
          if (err) {
            ...
            productData.thumbnail = mediaUrl + productData.thumbnail;
          }
        });
      } else {
        productData.thumbnail = defaultImage;
      ...
      productData.product_pdf = productData.product_pdf
        ?.map((pdf) => {
          ...
          try {
            if (pdf && pdf.trim() !== "" && fs.existsSync(pdfPath)) {
              return pdf;
            } else {
              return null;
            }
      ...
      if (product.selected_variants.length > 0) {
        product.selected_variants[0].forEach((valueIds, attributeId) => {
      ...
      productData.attribute_sets = product.attribute_sets.map((setId) => ({
        id: setId,
        name: attributeSetsMap[setId],
      }));

      productData.selected_variants = Object.keys(selectedVariants).reduce(
        (acc, attributeId) => {
          acc[attributeId] = selectedVariants[attributeId].map((value) => ({
            id: value.id,
            name: value.name,
          }));
          return acc;
      ...
      productData.selected_variants_ids = product.selected_variants_ids.map(
        (id) => ({
          id: id,
          name: attributeValuesMap[id],
        })
      ...
      const averageRating = ratingsCount ? totalRatings / ratingsCount : 0;
      ...
      const relatedProductData = await Promise.all(
        relatedProducts.map(async (product) => {
          ...
          if (
            ...
            try {
              await fsPromises.access(vendorImagePath, fs.constants.F_OK);
              productData.vendor_id.image =
                imageBaseUrl + productData.vendor_id.image;
            } catch (err) {
              productData.vendor_id.image = vendorDefaultImage;
  ...
  static filter = async (req, res) => {
    try {
      ...
      if ((min_price || max_price) && category_id) {
        ...
        products.forEach((product) => {
          ...
          if (
            ...
            product.category_id.status_id.equals(category_status._id)
          ) {
        ...
        products.forEach((product) => {
          ...
          if (
            ...
            if (product.category_id._id.equals(category_id)) {
              categoryDescription = product.category_id.description;
            }
          }
        });
        brandsArray = [...uniqueBrands];
      ...
      if (category_id) {
        ...
        min_unit_price = min_price ? parseFloat(min_price) : min_unit_price;
        max_unit_price = max_price ? parseFloat(max_price) : max_unit_price;
      } else {
        products.forEach((product) => {
          if (product.unit_price < min_unit_price) {
            min_unit_price = product.unit_price;
      ...
      const response = {
        ...
        price: {
          min_unit_price: min_unit_price === Infinity ? null : min_unit_price,
          max_unit_price: max_unit_price === -Infinity ? null : max_unit_price,
        },
        products: products,
        mediaUrl,
      };
    ...
    try {
      ...
      const responseData = products.map((product) => {
        ...
        let childCategories = categories.filter(
          (cat) =>
            cat.parent_id &&
            cat.parent_id.toString() === product.category_id.toString()
        );
        ...
        let stocks = productStocks.filter(
          (stock) => stock.product_id.toString() === product._id.toString()
        );
        ...
        if (!product.variant && productData.stocks.length > 0) {
          ...
          productData.current_stock = productData.stocks[0].current_stock;
        }
  ...
  static get_all_banners = async (req, res) => {
    try {
      ...
      const response = {
        ...
        subscription_banner: {
          image: await getImageUrl(banners?.subscription_banner?.image),
  ...
  static get_static_pages = async (req, res) => {
    try {
      ...
      const data = await StaticPage.find({ status_id: status._id }).populate(
        "status_id"
      );
      ...
      // Send the response with only the four required attributes
      return res.send({
        message: "Success",
        success: true,
        data: response,
